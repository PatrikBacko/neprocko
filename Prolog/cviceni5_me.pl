...
merge([], XR, XR) :- !.
merge(XL, [], XL) :- !.
merge([X|XL], [Y|XR], R) :- (X @>= Y -> merge([X|XL],XR,S), R = [Y|S]; merge(XL,[Y|XR],S), R = [X|S]).
...
vars(F, Vars) :-
    F =.. [Op, L, R], 
    member(Op, [and, or, imp, ekv]),
    vars(L, R1),
    vars(R, R2), 
    merge_Unique(R1, R2, Vars).


merge_Unique([], XR, XR) :- !.
merge_Unique(XL, [], XL) :- !.
merge_Unique([X|XL], [Y|XR], R) :- 
    (
    X @> Y -> merge([X|XL],XR,S), R = [Y|S]; 
    X == Y -> merge(XL,XR,S), R = [Y|S];
    merge(XL,[Y|XR],S), R = [X|S]
    ).
